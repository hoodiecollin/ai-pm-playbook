 ...
 *
 * A sub-issue requires its parent because its path contains the parent's state — closing an epic
 * moves its whole subtree. Passing the wrong parent, or none, is a programming error rather than
 * something to guess at: a silently mislocated issue would read as a delete plus a create.
 */
export function entityDir(e: BacklogEntity, parent?: BacklogEntity | null): string {
  switch (e.kind) {
    case "standalone":
    ...
    case "subissue": {
      if (!parent) {
        throw new Error(`#${e.number} is a sub-issue; its parent epic is required to render its path.`);
      }
      if (parent.number !== e.parent) {
        throw new Error(
          `#${e.number} names parent #${e.parent} but was rendered against #${parent.number}.`,
        );
      }
      return `${EPICS_DIR}/${segment(parent.state, parent.number)}/${SUBISSUES_DIR}/${segment(e.state, e.number)}`;
    }
  }
}

export function bodyPath(e: BacklogEntity, parent?: BacklogEntity | null): string {
  return `${entityDir(e, parent)}/${BODY_FILE}`;
}
